...
/*
==================================================
   Problem: Write a program to compare the sum of two matrices,
...
int sumOfMatrix(const Vec2DInt& vMatrix) {
    int sum = 0;
    for(const vector<int>& row : vMatrix) {
        for(int number : row) {
            sum += number;
...
bool areEqualMatrices(const Vec2DInt& vMatrix, const Vec2DInt& vMatrix2) {
    return sumOfMatrix(vMatrix) == sumOfMatrix(vMatrix2);
}
...
int main()
{
    ...
    if (areEqualMatrices(vMatrix, vMatrix2))
        cout << "\nYES: Sum of both matrices is equal.";
    else
        cout << "\nNo: Sum of both matrices is NOT equal.";
