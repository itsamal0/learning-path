...
==================================================
   Problem: Write a program to fill a matrix with random numbers,
            then sum each row in a separate array and print the results.
...
// Calculate the sum of all rows in the matrix into a normal array
void calculateAllRowSums(const vector<vector<int>>& vMatrix, int rowSums[]) {
...
void printRowSums(const int rowSums[], short rows) {
...
int main()
{
    ...
    calculateAllRowSums(vMatrix, rowSums);
